...
def:
  ...
  | FUN IDENT typec LBRA args RBRA block     { ASTFun($2, $3, $5, $7) }
  | FUN REC IDENT typec LBRA args RBRA block { ASTRec($3, $4, $6, $8) }
...
lval :
    IDENT                       { ASTLval($1) }
  | LPAR NTH lval expr RPAR     { ASTNth($3, $4) } // Offset de l'adresse de base ( &(a[i]) en c )
;
...
expr:
  ...
  | LPAR NTH expr expr RPAR           { ASTNthExpr($3, $4) } // la ième valeur de mon tableau (b[i] en c)
